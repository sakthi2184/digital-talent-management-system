...
export default function TaskManager() {
  ...
  const submit = async (e) => {
    ...
    try {
      if (editId) {
        await axios.put(`http://localhost:5000/api/tasks/update/${editId}`, form);
  ...
  const toggleStatus = async (task) => {
    const next = task.status === "Pending"
      ...
      ? "Completed" : "Pending";
    try {
      await axios.put(`http://localhost:5000/api/tasks/update/${task._id}`, { status: next });
          ...
          filteredTasks.map(task => (
            <div key={task._id} className="task-card">
              ...
              <div style={{ display: "flex", gap: "8px" }}>
                <button className="btn-sm-custom btn-toggle" onClick={() => toggleStatus(task)}>
                  ⟳ Status
                </button>
                <button className="btn-sm-custom btn-edit" onClick={() => startEdit(task)}>
                  ✏️ Edit
                </button>
                <button className="btn-sm-custom btn-delete" onClick={() => deleteTask(task._id)}>
                  🗑 Delete
                </button>
              </div>
            </div>
